...
import TransactionAddItem from '../components/Transactions/TransactionAddItem';
...
export default function Transactions() {
  ...
  const [rowperpage, setRowperpage] = useState(Math.round((window.innerHeight * 0.52) / (70 + 16)));
  const [totalPages, setTotalPages] = useState(Math.ceil(filtereddata.length / rowperpage));
  ...
  useEffect(() => {
    if (addTransaction) {
      setRowperpage(Math.round((window.innerHeight * 0.52) / (70 + 16)) - 1);
      setPage(1);
    } else {
      setRowperpage(Math.round((window.innerHeight * 0.52) / (70 + 16)));
      setPage(1);
  ...
  useEffect(() => {
    ...
    if (filter.search !== '') {
      ...
      filteredData = filteredData.filter((transaction) => transaction.date.toISOString().slice(0, 7) === filter.date.toString());
      ...
      filteredData = filteredData.filter((transaction) => transaction.category.toLowerCase() === filter.category.toLowerCase());
    }
  ...
  return (
    <main id="Transactions">
      <div className="top">
        <h2>Transactions</h2>
        <button onClick={() => setAddTransaction(true)}>+ Add Transactions</button>
      </div>
      <div className="content">
        <TransactionsFilters setFilter={setFilter} filter={filter} />
        {addTransaction && <TransactionAddItem setAddTransaction={setAddTransaction} />}
